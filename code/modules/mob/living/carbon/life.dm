	...
	if(!stat)
		...
		if(world.time > mob_timers["painstun"])
			...
			if(lying || IsKnockdown())
				...
				if(painpercent >= 100)
					if(prob(25) && (HAS_TRAIT(src, TRAIT_PSYDONIAN_GRIT) || STAWIL >= 15) && !HAS_TRAIT(src, TRAIT_NOPAIN)) // PSYDONIC WEIGHTED COINFLIP. TWEAK THIS AS THOU WILT. DON'T LET THEM BE BROKEN, PSYDON WILLING. THROW CON-MAXXERS A BONE, TOO.
						...
						emote("painmoan")
						return
					if(prob(probby) && !HAS_TRAIT(src, TRAIT_NOPAIN) && !has_status_effect(/datum/status_effect/buff/psyhealing))
						Immobilize(10)
						emote("painscream")
						stuttering += 5
