...
/obj/effect/proc_holder/spell/targeted/soulspeak/cast(list/targets,mob/user = usr)
	...
	for(var/mob/living/carbon/spirit/S in GLOB.player_list) //CC Edit mob_list -> player_list
		if(S.summoned)
			continue
		if(!S.client)
			continue
	...
	if(!pickedsoul)
		...
		return
	for(var/mob/living/carbon/spirit/P in GLOB.player_list) //CC Edit mob_list -> player_list
