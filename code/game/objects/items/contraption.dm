...
/obj/item/contraption/linker/equipped(mob/user, slot)
	...
	if(active_item)
		return
	else if(slot == SLOT_HANDS || slot == ITEM_SLOT_HANDS)
...
/obj/item/contraption/pick/drill/equipped(mob/user, slot)
	..()
	if(active_item)
		return
	else if(slot == ITEM_SLOT_HANDS || slot == SLOT_HANDS)
		if (user.get_skill_level(/datum/skill/craft/engineering) >= 4)
